...
for (let row = 0; row < 9; row++) {
  for (let col = 0; col < 9; col++) {
    ...
    if (row != 8 && col != 0) {
      ...
      square.style.animationDelay = `${index * 0.08}s`;
    ...
    if (col == 0 && row != 8) {
      ...
      internal_col_number.style.animationDelay = `${internal_row * 0.1 + 1}s`;
    ...
    internal_row_number.style.animationDelay = `${internal_col * 0.1 + 1.8}s`;
